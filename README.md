...
1. **下载技能文件**
   - 获取 `wow-translation.skill` 文件（这是一个包含所有术语和脚本的完整包）
2. **让 AI 助手帮忙安装**
   - 在你的 AI 工具中输入：
     ```
   ...
   - AI 助手会自动识别文件类型并完成安装
3. **开始使用**
   - 安装完成后，你就可以直接使用 `/wow-translation` 命令来触发翻译功能

### 技术说明

- `.skill` 文件是一个完整的 ZIP 包，包含所有必要的术语库和处理脚本
- 支持的 AI 工具包括：Claude Code、OpenClaw、Hermes、Cursor、Trae 等
- 安装过程会自动将文件放置到正确的技能目录中
- 无需手动创建目录或配置文件，AI 助手会处理所有细节
